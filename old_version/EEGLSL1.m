classdef EEGLSL < handle
    ...
    % >> self.plot_size = self.sampling_frequency * 4;
    % >> self.exp_data_length = self.plot_size;
    % >> ds1 = self.CreateNewDS('1',ones(1,32),[10 15]);
    % >> ds2 = self.CreateNewDS('2',ones(1,32),[7 9]);
    % >> cds = self.CreateNewCompositeDS(ds1,ds2,'+','ds1 + ds2');
    % >> d = ones(32,10);
    % >> ds1.Apply(d,0);
    % >> ds2.Apply(d,0);
    % >> cds.Apply(d,0);
    % >> ds1.ring_buff.raw(ds1.ring_buff.fst:ds1.ring_buff.lst)
    %
    %ans =
    %
    %    0.0525    0.2639    0.5338    0.4209   -0.4015   -1.5716   -2.0974   -1.1391    1.1192    3.3253
    %
    % >> ds2.ring_buff.raw(ds2.ring_buff.fst:ds2.ring_buff.lst)
    %
    %ans =
    %
    %    0.0037    0.0226    0.0659    0.1223    0.1577    0.1302    0.0135   -0.1838   -0.4105   -0.5830
    %
    % >> cds.ring_buff.raw(cds.ring_buff.fst:cds.ring_buff.lst)
    %
    %ans =
    %
    %    0.0562    0.2864    0.5997    0.5432    0.5592    1.7018    2.1108    1.3229    1.5296    3.9084
    %
    ...
    methods
        ...
        function UpdateFeedbackSignal(self)
            if length(self.derived_signals) > 1 %%once we created some derived signals
                ...
                if self.recording
                    try
                        fb = zeros(self.current_window_size,self.fb_manager_size);
  ...
  if self.ssd < self.next_protocol -1
    ...
    while true
        ...
        if ~ishandle(csp_figure)
            ...
            %self.csp_chs{d} = self.SelectChannelsForCSP(data_names{d});
            bl_data_set = self.ReduceDataSet(bl_edited,self.csp_chs{end});
            %csp_data_set = self.ReduceDataSet(data_sets{d},self.csp_chs{d});
