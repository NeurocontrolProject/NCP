...
load('C:\Math\Neuroloop\RealTime\cbi\bci_expresult_LSL32_first_2603_first_imag_T20.mat')
%load('C:\Math\Neuroloop\RealTime\cbi\bci_expresult_LSL32_first_2603_first_real_T20.mat') 
data_cur = data.data;
...
%         title([num2str(i), ', ', num2str(j)]);
         ...
         for k=1:size(y_data,1)
             y_data(k,:) = conv(y_data(k,:),ones(1,100),'same');
         end;
         
         [C,err,P,logp,coeff] = classify(y_data', y_data', y_states,'mahalanobis');
         A(i,j) = 1-err;
%         fprintf('LDA error: %f\n', err);
% 
